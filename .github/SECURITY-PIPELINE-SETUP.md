...
- **Current baseline:** `build.gradle.kts` is `2.2.0` and the highest tag is `v2.2.0`. Once you publish the `v2.2.0` release yourself, the first automated security release is **`v2.2.1`**, then `v2.2.2`, … (each driven by a merged `security`-labeled PR).
