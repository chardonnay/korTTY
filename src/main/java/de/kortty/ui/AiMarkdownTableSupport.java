...
/**
 * Helpers for rendered markdown tables and clipboard serialization.
 */
final class AiMarkdownTableSupport {

    private AiMarkdownTableSupport() {
    }

    static RenderedMarkdownTable buildRenderedTable(List<List<String>> rawRows) {
    ...
    static String toTsv(RenderedMarkdownTable table) {
    ...
    static String toColumnText(RenderedMarkdownTable table, int columnIndex) {
        if (table == null) {
            return "";
        }
    ...
    static String toCellText(RenderedMarkdownTable table, int rowIndex, int columnIndex) {
    ...
    static boolean isBoldMarkdown(String value) {
        String trimmed = value != null ? value.trim() : "";
        return trimmed.startsWith("**") && trimmed.endsWith("**") && trimmed.length() >= 4;
    }

    static String stripInlineMarkdown(String value) {
        if (value == null) {
            return "";
        }
        return value
    ...
    static boolean isNumericLike(String value) {
        if (value == null) {
            return false;
        }
    ...
    record RenderedMarkdownTable(List<String> header, List<List<String>> rows) {
    }
}
