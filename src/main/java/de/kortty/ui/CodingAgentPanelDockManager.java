...
/**
 * Holds where the Coding Agents panel is docked: HIDDEN (default), or to the LEFT/RIGHT side of
 * the main window. A clone of {@link SessionJournalLivePanelDockManager} with the panel's own
 * width bounds ({@link CodingAgentPanelDefaults}).
 *
 ...
 */
public class CodingAgentPanelDockManager {

    /** Where the panel sits; HIDDEN keeps the last docked side for the show/hide toggle. */
    ...
    // Derived from CodingAgentPanelDefaults so the docked-width bounds have a single source of truth
    // shared with GlobalSettings.
    public static final double MIN_WIDTH = CodingAgentPanelDefaults.MIN_WIDTH;
    public static final double MAX_WIDTH = CodingAgentPanelDefaults.MAX_WIDTH;
    public static final double DEFAULT_WIDTH = CodingAgentPanelDefaults.DEFAULT_WIDTH;
    ...
    public CodingAgentPanelDockManager() {
    }
    ...
    /** The side the panel was docked to most recently (RIGHT until it was ever docked). */
    public Placement getLastDockedSide() {
        return lastDockedSide;
    }
