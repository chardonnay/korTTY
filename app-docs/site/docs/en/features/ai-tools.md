...
- **Input hardening** — A second collapsible panel that asks the AI to build an input-validation guard block into the generated script: parameter allowlists and length limits, file format checks, a maximum input-file size controlled by the adjustable `MAX_FILE_SIZE` script variable, security warnings in the script's log, and a `KORTTY_FORCE=1` override. The guard checks file metadata before it reads content, and `0` means unlimited. Strictly opt-in — the master check box starts unticked. See [Input hardening](../reference/input-hardening.md).
...
- **Mermaid diagram** — Optionally include a Mermaid flowchart depicting the script logic. Its dedicated source-only request omits AI Skills and knowledge-store excerpts and sets the request-scoped **Reasoning** value to `none` only when the provider advertised that value; the saved profile remains unchanged. A working spinner is shown while the diagram is generated.
