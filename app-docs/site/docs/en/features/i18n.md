...
!!! warning "Yandex keys stored by an earlier korTTY version"
    The old Yandex Translate API v1.5 has been shut down by Yandex, and korTTY now talks to the
    Yandex Cloud Translate v2 endpoint instead. A v1.5 key stored in an earlier korTTY version no
    longer works: create a service account in the Yandex Cloud console, give it the
