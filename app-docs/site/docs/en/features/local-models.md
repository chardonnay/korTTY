...
- **Local AI** assigns profiles to the Text/translation and Coding roles and to the session journal's AI summaries, selects the RAG embedding model, stores an optional Hugging Face token, and records the llama.cpp runtime-update policy.
...
The [Hugging Face](https://huggingface.co/docs/hub/index) browser searches model repositories and shows repository, format, architecture, available quantizations, license, selected-quantization size, context length, publication age, and a hardware estimate. The **Age** column reflects when the repository was first published on the Hub — shown as `today`, days, months, or years, and `—` when the Hub does not report a creation date. On Apple Silicon a **Format** selector next to the search field switches between **GGUF + MLX** (default), **GGUF**, and **MLX**; each enabled format keeps its own cursor pagination, and the **Format** column identifies every row. Other platforms search GGUF only.

Click a column header to sort the results by that column — a second click reverses the direction, and the header shows the active sort arrow. Size, context and age sort numerically (repositories without a reported creation date sort last), and the hardware estimate sorts from best to worst fit; the chosen order persists while background metadata loads complete.
...
- **The MLX runtime package** is separate from both the application installer and the llama.cpp runtime: a pinned relocatable CPython plus a hash-locked `mlx-lm` wheel set, built in CI and published through the same Ed25519-signed index mechanism as the llama.cpp packages and resolved through the same latest release. Install, update, locally import, or remove it in the **Runtimes** table; without an installed MLX runtime, MLX models stay registered but cannot start.
...
The installed-models table also carries an **Age** column: the model's upstream publication date, captured once at install time. Models installed before this column existed and manually imported GGUF files have no captured date and show `—`.
...
In **AI Manager > Local AI**, choose separate profiles for **Text and translation** and **Coding**. Leaving either choice at **Use default AI profile** preserves the normal fallback. The same pane also holds a **Session journal profile** selector used only for the [session journal](session-journal.md)'s AI summaries — the Text/Coding roles do not apply there; leave it on the default profile unless the journal should run on a different model. Terminal summarization, problem solving, questions, descriptions, and translation use the Text role; snippet generation, completion, analysis, security fixes, and workflow generation use the Coding role.
...
Chat sidecars keep the server's default reasoning parsing, which correctly handles both `<think>`-style reasoning models (whose chain-of-thought is returned separately) and channel/harmony models such as gpt-oss (which require that parsing to produce a final answer at all). The pinned server can still occasionally answer a whole reply with an HTTP 500 (“does not match the expected … format”) when a reply parses ambiguously; korTTY retries a local server error, an empty reply, or a reasoning-only reply once automatically before reporting it (for the dedicated snippet/workflow diagram request, only a server error is retried), and defensively strips any reasoning markers that leak into the content.

The runtime build is independent of the application installer. `gradle/llama-cpp-pins.properties` pins the upstream llama.cpp tag, full commit, and source-archive SHA-256 (plus the history of shipped pins), while `build.gradle.kts` keeps the API-contract version and korTTY package revision. The korTTY repository detects candidates and validates its source-side native matrix but holds no publication token or runtime signing key. The public [korTTY llama.cpp runtime channel](https://github.com/chardonnay/kortty-llama-runtimes) owns explicit human-dispatched stable publication: it rebuilds the reviewed source commit, runs authentication, chat/completion, embeddings, JSON-schema, sleep/wake, and parallel-sidecar smoke tests on every package plus a separate Qdrant contract, and publishes immutable descriptors with its own scoped `github.token` only after entering the protected signing environment. CUDA is not a v1 backend.
