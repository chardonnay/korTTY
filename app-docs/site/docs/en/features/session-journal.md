...
- **The timeline** — new cards (AI summaries, notes, screenshots) and edits appear moments after they happen, without losing your scroll position. A [terminal AI-agent](ai-assistant.md) run adds its own **AI agent** card the moment it finishes: your prompt as the title and the agent's final answer as the body — so agent work is part of the journal record even though the summarizer treats the agent's inline terminal text as noise.
