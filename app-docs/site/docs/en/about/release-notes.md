...
- **Robust local reasoning replies** — chat sidecars run with reasoning-format parsing disabled and korTTY separates a reasoning model's inline chain-of-thought itself, preventing the pinned llama.cpp server's strict chat-format parser from failing whole replies (HTTP 500 “does not match the expected peg-native format”, e.g. from the snippet code-analysis actions) when generation stops inside a thinking block. Local server errors, empty replies, and reasoning-only replies are retried once automatically.
