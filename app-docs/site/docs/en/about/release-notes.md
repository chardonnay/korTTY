...
- **Terminal-only fullscreen now shows the whole korTTY window, not a stretched terminal** — **View → Terminal-only Fullscreen** (++f12++) keeps the whole korTTY window — menu bar, tabs, and status bar included — at the size it had before entering the mode (falling back to the configured window geometry when the window was already fullscreen) and centers it on an empty fullscreen background, so the desktop and other windows stop competing for attention.
