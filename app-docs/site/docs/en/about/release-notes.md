...
- **Safer PlantUML cache and packaged rendering** — the first-use JAR is checked against a fixed SHA-256, obsolete versions and stale render directories are cleaned, and packaged rendering reuses korTTY's launcher as a cancellable private worker even though the stripped runtime has no `bin/java`; Graphviz remains optional for the activity and sequence diagrams korTTY generates.
