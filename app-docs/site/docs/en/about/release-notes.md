...
- **Independently updated, revocation- and rollback-safe llama.cpp packages** — a daily workflow opens candidate pin PRs and builds CPU, Metal, and Vulkan packages from the exact tag/commit/archive SHA-256 in Gradle. Authenticated chat, embeddings, JSON schema, sleep/wake, parallel-sidecar, and pinned-Qdrant smokes must pass before a human-dispatched stable promotion signs a cumulative `runtime-index-v1.json`; the Local Models manager installs or updates the preferred backend, warns before switching for a Metal/Vulkan model, and startup policy supports Off/Notify/Automatic Stable. Verified withdrawals persist a denylist and package quarantine marker, stop/rebind affected sidecars and remove unsafe rollback candidates; new packages activate only while idle and remain pending until the first real GGUF/API start, whose failure restores the newest healthy non-revoked version. The two newest confirmed healthy versions remain available.
