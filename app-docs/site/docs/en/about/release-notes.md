...
- **Terminal-only fullscreen no longer stretches the terminal across the screen** — **View → Terminal-only Fullscreen** (++f12++) now keeps the terminal at the size it had before entering the mode (falling back to the configured window geometry when the window was already fullscreen) and centers it on an empty fullscreen background, so a single terminal can be focused without the desktop showing behind it.
