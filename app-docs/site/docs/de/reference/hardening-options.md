...
**Härtungsoptionen** sind eine Reihe von Techniken in Produktionsqualität, die Sie von der KI in ein Skript integrieren lassen können, das sie generiert oder neu schreibt. Anstatt jedes Mal die gleiche Anweisung („Fehlerbehandlung hinzufügen“, „Wiederausführbar machen“, „Geben Sie ihm einen `--help`“) von Hand zu schreiben, kreuzen Sie die gewünschten Techniken an und KorTTY wandelt jede einzelne in eine präzise Regel um, die an die KI-Eingabeaufforderung angehängt wird. Das Ergebnis ist ein Skript, das sich wie etwas verhält, das ein sorgfältiger Ingenieur liefern würde: Es schlägt laut statt lautlos fehl, bereinigt sich selbst, protokolliert, was es tut, und kann ohne Überraschungen erneut ausgeführt werden.
...
Bei **Robustheit verbessern** und **Benutzerdefinierte Verbesserung** werden aktive Härtungsregeln auf das **vollständige Snippet** angewendet, auch wenn Sie nur einen Bereich markiert haben. Dies ist erforderlich, da Optionen wie der strikte Modus, ein Konfigurationsblock, `--help` und eine abschließende Zusammenfassung den Prolog oder Epilog und nicht nur die ausgewählten Zeilen ändern müssen. Wenn Sie **Löschen** verwenden und die Eingabe-Härtung deaktiviert lassen, bleibt die normale Verbesserung auf den ausgewählten Bereich beschränkt. Das Complete-Snippet-Ergebnis wird weiterhin in der Vorher-/Nachher-Überprüfung angezeigt und verworfen, wenn die KI unveränderten Code mit einer Auslassungsmarkierung abkürzt.

Unterhalb der Optionen helfen Ihnen drei Schaltflächen bei der Verwaltung der Auswahl:

- **Alle** – kreuzen Sie jede Option an.
- **Clear** – alle Optionen deaktivieren (ohne Härtungsregeln generieren/anwenden).
...
- **Verhaltenserhaltende Härtung** (die ersten sieben) fügt lediglich Dokumentation, Protokollierung, Struktur und Sicherheitsnetze hinzu. Sie machen das Skript stabiler und leichter lesbar, **ohne seine eigentliche Funktion zu ändern**. Diese können bei fast jedem Drehbuch bedenkenlos aktiviert bleiben.
...
### Verhaltenserhaltendes Härten

#### Strikter Modus (Abbruch bei Fehler)

- **Wofür es ist** – Stoppen Sie das Skript, sobald etwas schief geht, anstatt blind in einem halb gescheiterten Zustand fortzufahren.
...
- **Ansible-Playbooks** – Überprüfen Sie die Voraussetzungen mit `assert`/`failed_when`, damit die Wiedergabe aufgrund eines fehlerhaften Zustands sofort fehlschlägt.
...
- **Imperative Skripte** – Senden Sie zeitgestempelte Protokollnachrichten an stderr und unterstützen Sie ein `--verbose`/`-v`-Flag.
- **Ansible-Playbooks** – Verwenden Sie das Modul `debug` für die Fortschrittsausgabe (sichtbar mit `-v`).
...
- **Ansible-Playbooks** – Beenden Sie mit einer `debug`-Zusammenfassung der Änderungen.

#### Style-Guide / Linter reinigen

- **Wofür es ist** – Erstellen Sie Code, der den Standard-Linter der Sprache erfüllt, sodass er konsistent liest und häufige Fehler vermeidet.
- **Imperative Skripte** – Befolgen Sie den Sprachstil-Guide und halten Sie ihn sauber (z. B. ShellCheck-clean für Bash).
...
- **Ansible-Playbooks** – Fügen Sie vor jeder Änderung `pre_tasks`/`assert`-Prüfungen auf erforderliche Berechtigungen, Pakete und Konnektivität hinzu.

#### Idempotency (abgeschlossene Schritte überspringen)

- **Wozu es dient** – Machen Sie das Skript sicher für die zweite Ausführung: Schritte, die bereits ausgeführt wurden, werden erkannt und übersprungen, anstatt wiederholt zu werden oder einen Fehler zu verursachen.
...
- **Ansible-Playbooks** – Stellen Sie sicher, dass das Playbook vollständig idempotent ist (sichere Wiederholung; verlassen Sie sich auf Modul-Idempotenz und `creates`/`removes`).

#### Abgesicherter Modus (`--dry-run` + bestätigen)

- **Wozu es dient** – Lassen Sie den Bediener eine Vorschau dessen anzeigen, was das Skript tun würde, ohne Änderungen vorzunehmen, und bitten Sie ihn um Bestätigung, bevor etwas destruktives geschieht.
- **Imperative Skripte** – Unterstützt ein `--dry-run`-Flag, das beabsichtigte Aktionen ohne Ausführung ausgibt und vor destruktiven Vorgängen bestätigt (unterdrückbar mit `--yes`).
