...
!!! tip "Unbeaufsichtigter erster Start (Automatisierungs-/Test-VMs)"
    ...
    Bei diesem ersten Start richtet korTTY automatisch das Standard-Master-Passwort `kortty-auto` ein und entsperrt den Tresor, sodass für Skript- oder CI-Ausführungen keine Interaktion erforderlich ist. Wenn Sie später die automatische Anmeldung für ein solches Profil deaktivieren, ist `kortty-auto` das einzugebende Passwort – ändern Sie es sofort über **Master-Passwort ändern**, wenn das Profil weiterbestehen soll. Nur für Einweg-/Testumgebungen gedacht – siehe Sicherheitswarnung oben.
...
!!! note "Schaltfläche „Master-Passwort ändern“"
    ...
    Das neue Passwort wird erst übernommen, wenn jeder Wissensspeicher migriert wurde. Bei einem Fehler auf halbem Weg bleibt also das alte Passwort in Kraft, anstatt Sie für die Hälfte Ihrer Daten zu sperren. Wenn ein einzelnes Geheimnis nicht migriert werden kann (z. B. eines, das mit einem anderen Passwort verschlüsselt wurde), lässt korTTY diesen Wert unverändert, meldet, wie viele Elemente betroffen waren, und vermerkt sie im Protokoll – diese Geheimnisse behalten das alte Passwort und müssen manuell neu eingegeben werden. Wenn die automatische Anmeldung aktiviert ist, wird das gespeicherte Passwort in `~/.kortty/master.autounlock` in das neue Master-Passwort umgeschrieben, sodass die automatische Entsperrung nach der Änderung weiterhin funktioniert.