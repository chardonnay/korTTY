...
- **Verbesserung der Lesbarkeit/Robustheit/Leistung** – Schreibt den **ausgewählten** Codebereich in Richtung eines Ziels ohne unabhängige Änderungen um. *Verbesserung der Robustheit* bietet zusätzlich [Härtemöglichkeiten](../reference/hardening-options.md) und das Opt-In [Eingabe-Härtung](../reference/input-hardening.md) bewachen, bevor es läuft.
...
- **Benutzerdefinierte Verbesserung…** – Schreibt den ausgewählten Codebereich gemäß einer von Ihnen eingegebenen Freitextanweisung neu [Härtemöglichkeiten](../reference/hardening-options.md) Und [Eingabe-Härtung](../reference/input-hardening.md) Paneele.
...
- **Automatisch generieren** – Ein Kontrollkästchen in der Kopfzeile des Bereichs steuert, ob die Diagrammanforderung beim Öffnen des Fensters automatisch startet. Deaktivieren Sie das Häkchen, um die automatische KI-Anfrage vollständig zu überspringen – im Bereich wird dann stattdessen ein Hinweis angezeigt, und **Neugenerieren** bleibt die manuelle Methode zum Anfordern des Diagramms. Wenn Sie das Kontrollkästchen aktivieren, während das Fenster geöffnet ist, wird das Diagramm sofort abgerufen. Die Auswahl wird sitzungsübergreifend gespeichert (Standard: Ein) und hat keine Auswirkungen auf das eigenständige Fenster [Diagram](#mermaid-diagramme), das gespeicherte Diagramme ohne AI-Anfrage rendert.
...
Durch das Ändern der Fertigkeiten wird **nicht** sofort eine erneute Analyse durchgeführt – der neue Satz wird beim nächsten **Wiederholen** angewendet, sodass ein bewusster Klick die Berichts- und Diagrammanforderungen mit genau den von Ihnen ausgewählten Fertigkeiten erneut ausführt. Fertigkeiten, die Sie hier einschließen, werden unabhängig vom konfigurierten *Ziel* der einzelnen Fertigkeiten gesendet. Die Zeile wird nur angezeigt, wenn mindestens eine KI-Fähigkeit aktiviert ist.
...
Wenn Sie auf **Ausgewählte anwenden** klicken, schließt korTTY das Analysefenster, bevor der Anwendungsfluss gestartet wird, sendet die angekreuzten Verbesserungen und Abhängigkeitsvorschläge (plus etwaige Härtungsoptionen) in einer Anfrage an die KI und bringt das Fenster *Verbesserungen anwenden – Änderungen überprüfen* in den Vordergrund. Die Anforderung erfordert ein vollständig neu geschriebenes Skript, einschließlich aller unveränderten Abschnitte, die vom Original kopiert wurden. Eine KI-Antwort, die eine Auslassungsmarkierung wie `rest unchanged` einführt, wird abgelehnt, bevor die Vorschau geöffnet wird, und der Editorinhalt bleibt unberührt. Eine gültige Antwort zeigt das ursprüngliche und das umgeschriebene Skript nebeneinander, mit hervorgehobenen geänderten Zeilen und den jeweiligen Änderungsgründen, genau wie bei der Sicherheitsüberprüfung unten; Der Editor bleibt unverändert, bis Sie in dieser Vorschau **Änderung übernehmen** bestätigen. Jeder ausgewählte **Skript-Header** wird dem Ergebnis vorangestellt, bevor es angezeigt wird. Ein eigenständiger Header – ohne angekreuzte Verbesserungen, Abhängigkeiten oder Härtung – wird direkt eingefügt, ohne einen KI-Roundtrip, und immer noch zuerst als Vorher/Nachher-Vorschau angezeigt.
