...
- **Custom improvement…** — Rewrites the selected code region following a free-text instruction you type. It displays the same two optional rule panels: [Härtemöglichkeiten](../reference/hardening-options.md), [Eingabe-Härtung](../reference/input-hardening.md). Wie bei *Robustheit verbessern* schreibt KorTTY das komplette Snippet neu, wenn eine Härtungsregel aktiv ist.
