...
- **Reduzierbare Optionsabschnitte** – die optionalen Einstellungen (Verbindungszeitlimit, Terminaldarstellung, Terminaleffekt, KI) sind in zusammenklappbaren Abschnitten gruppiert, sodass das Dialogfeld kompakt geöffnet wird; Das Verbindungs-Timeout wird jetzt mit festen Standardwerten geöffnet (10 s / 0 Wiederholungen). Das Formular scrollt innerhalb des Dialogs, wenn erweiterte Abschnitte über den Bildschirm hinausgehen, und der Dialog merkt sich bei Neustarts **merkt sich, welche Abschnitte Sie erweitert gelassen haben**.
- **Verbindungs-Skills-Auswahl** – Wählen Sie KI-Skills für die neue Verbindung vorab aus: eine globale Suche (`*`-Platzhalter), All/Löschen-Schalter und eine Schaltfläche **Speichern**, die die aktuelle Auswahl als Standard für jede neue Verbindung beibehält.
...
- **AI Manager merkt sich sein Fenster** – das AI Manager-Dialogfeld wird jetzt in der Größe und Position wieder geöffnet, an der Sie es zuletzt verlassen haben, anstatt jedes Mal neu gestartet zu werden.
...
- **Opt-in-Ressourcenprofil** – über eine neue Registerkarte **Konfiguration → Globale Einstellungen → Ressourcen** können Sie diesen geringen Platzbedarf bei Bedarf gegen mehr Ressourcen Ihres Computers eintauschen: **Hoch** erhöht den Heap auf etwa die Hälfte Ihres RAM und **Maximum** auf etwa drei Viertel mit dem Z-Garbage-Collector mit niedriger Pause. Der Standardwert (**Balanced**) bleibt unverändert. Siehe [Ressourcen](../reference/settings/resources.md).
- **Kleinere Downloads und Installationen** – die gebündelte Formatter-Laufzeitumgebung und die eingebettete Java-Laufzeitumgebung wurden gekürzt und komprimiert, das macOS-Disk-Image verwendet jetzt eine stärkere Komprimierung (LZMA) und der Offline-Anleitung enthält keine Entwickler-Quellkarten oder ungenutzten Suchkomponenten mehr. Installationen schrumpfen um etwa 150 MB, Downloads um 25–40 %.
