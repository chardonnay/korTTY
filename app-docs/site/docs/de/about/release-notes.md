...
- **Sichererer PlantUML-Cache und verpacktes Rendering** – die Erstverwendungs-JAR wird anhand eines festen SHA-256 überprüft, veraltete Versionen und veraltete Renderverzeichnisse werden bereinigt und beim verpackten Rendering wird der Launcher von korTTY als stornierbarer privater Worker wiederverwendet, obwohl die entfernte Laufzeit kein `bin/java` hat; Graphviz bleibt für die von korTTY generierten Aktivitäts- und Sequenzdiagramme optional.
