...
*v2.17.0* | link:../README.adoc[Back to README]
...
* *AI Code > AI Complete* (also `Shift+Tab` at the end of a non-empty line, or `Ctrl+Space` anywhere) - Opens a completion list at the cursor with candidates harvested from the snippet itself (arrays, variables, functions, language idioms); when an AI profile is configured, up to five AI candidates join the list as they arrive. `Tab`/`Enter` inserts the selected entry, `Esc` closes the list.
* *AI Code > Auto AI Complete* - Shows the AI's suggestion as ghost text after you pause at the end of a non-empty line. `Tab` accepts it, `Esc` dismisses it. This is off by default and only stays enabled for the current editor session; its one-time data notice now appears once per application start rather than once per editor.
