...
* *Selectable app designs* for JavaFX windows and dialogs: Normal, Matrix Terminal, Holographic Interface, Klingon Tactical, and Elegant Dark
...
* *Terminal-only fullscreen* - Press F12 or use *View > Terminal-only Fullscreen* to hide the menu bar, status bar, dashboard, local file browser, and other window chrome so only the terminal remains visible. *View > Hide terminal scrollbars in fullscreen* can also hide terminal scrollbars while fullscreen mode is active. Press F12 again to restore the previous fullscreen and UI state.
