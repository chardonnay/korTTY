...
* *JDK 25* (the native packaging tasks call the JDK 25 `jpackage` executable from `PATH`; newer JDKs are not currently supported for packaging)
...
* *Local build:* The first time you run `./gradlew build`, the task `cloneSithtermfx` clones the link:https://github.com/chardonnay/SithTermFX[SithTermFX] repo at tag `v1.2.1` into `vendor/sithtermfx`, then `installSithtermfxLocal` runs `mvn -q -DskipTests install` there. The artifacts are installed to your local Maven repo (`mavenLocal()`). Requirements: *Git* and *Maven* on your PATH.
...
* Use a full *JDK 25* for the target CPU and ensure both `java` and `jpackage` from that JDK are in `PATH`. The Gradle toolchain can provision a compiler, but the packaging tasks invoke `jpackage` directly from `PATH`.
...
* Build on the target operating system. The package CPU architecture follows the JDK that runs Gradle, so use an x86_64 JDK for Intel packages and an arm64 JDK for ARM packages.
...
`java` and `jpackage` must report major version 25.
