...
* *Terminal AI Commands*: Intercept configurable local shortcut commands such as `agent`, `agent-ask`, and `agent-plan` directly at the shell prompt
...
KorTTY keeps profile selection, quota tracking, and result handling local while sending only the reviewed terminal selection and optional prompt to the configured OpenAI-compatible endpoint.
