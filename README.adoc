...
image::app-docs/diagrams/architecture.png[KorTTY architecture,align=center]
...
* *Terminal-only Fullscreen*: Press *F12* or use *View → Terminal-only Fullscreen* to hide menu/status chrome, dashboard, and file browser while keeping only the terminal visible; toggling it again restores the previous fullscreen and UI state
...
image::app-docs/diagrams/ai-api-integration.png[KorTTY AI API integration,align=center]
...
image::app-docs/diagrams/terminal-effect-plugin-flow.png[Terminal effect plugin lifecycle,align=center]
...
image::app-docs/diagrams/modules-overview.png[Modules overview,align=center]
...
image::app-docs/diagrams/session-lifecycle.png[Terminal session lifecycle,align=center]
...
image::app-docs/diagrams/connection-flow.png[Connection flow,align=center]
...
image::app-docs/diagrams/credential-flow.png[Credential and authentication flow,align=center]
...
image::app-docs/diagrams/backup-restore.png[Backup and restore flow,align=center]
...
image::app-docs/diagrams/jump-server-flow.png[Jump server connection flow,align=center]
...
For distraction-free terminal work, *F12* toggles *View → Terminal-only Fullscreen*. This mode hides the menu bar, status bar, dashboard, local file browser, and other window chrome, then restores the previous fullscreen and UI state when toggled off.
...
The documentation uses PlantUML diagrams (`.puml` files in `app-docs/diagrams/`). To regenerate PNG images from them (e.g. after editing a diagram), use the provided script. Requires Java and Graphviz.
...
PNGs are written to `app-docs/diagrams/` and are embedded in the AsciiDoc documentation.
