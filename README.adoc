...
* *AI Profiles, Defaults, Reasoning, and Quotas*: Manage multiple named AI profiles with per-profile endpoint/model/API key, optional reasoning effort where supported by the configured API/model, choose a default AI profile, configure tokenizer strategy, token budgets, warning thresholds, reset cycles, and code-text language defaults
