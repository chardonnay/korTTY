...
*v2.0.0* — A modern SSH client with JavaFX interface, tabbed terminals, SFTP tooling, and OpenAI-compatible AI assistance.
...
* *Theme Profiles*: Multiple built-in color profiles (including IntelliJ-inspired themes) with live preview, one-click apply in Global Settings, and dedicated Terminal AI agent panel colors
...
* *Snippet Manager*: Create, search, favorite, and organize reusable snippets with syntax highlighting, line numbers, one-liner export, and JSON/XML/YAML import/export
* *AI-assisted Snippet Editing*: Generate snippet name/description/language from code, correct description text, transform selected comments and user-facing output text, create technical descriptions, and request alternative implementations
...
* *AI Profiles, Defaults, and Quotas*: Manage multiple named AI profiles with per-profile endpoint/model/API key, choose a default AI profile, configure tokenizer strategy, token budgets, warning thresholds, reset cycles, and code-text language defaults
...
* *AI Agent & Planning*: Start AI Agent and AI Planning from *Tools* or the terminal context menu, choose terminal or chat execution targets, and use dedicated planning/run tabs with transcript export
* *Terminal AI Activity Feed*: Terminal-targeted agent runs show an inline activity panel with status dots, task timing, collapsible details, history navigation, rerun, resize, font controls, cancel, sudo password prompts, and run export to Markdown/Text/YAML/XML/JSON/PDF/Asciidoctor
* *Terminal AI Commands*: Intercept configurable local shortcut commands such as `agent`, `agent-ask`, and `agent-plan` directly at the shell prompt, including the active remote working directory for terminal-targeted tasks
...
KorTTY keeps profile selection, quota tracking, and result handling local while sending only the reviewed terminal selection and optional prompt to the configured OpenAI-compatible endpoint for selection-based AI actions. Terminal-agent runs send the agent goal together with a compact SSH-session snapshot, including the active remote working directory captured from the shell.
...
To *manually trigger* a release build (e.g. after moving the tag without creating a new release), go to *Actions → Build Release Binaries → Run workflow*, choose branch `main`, set *Ref* to the tag (e.g. `v2.0.0`) and *Version* to the numeric version (e.g. `2.0.0`).
...
[source,text]
----
build/jpackage/korTTY-2.0.0.dmg
----
...
[source,text]
----
build\jpackage\korTTY-2.0.0.msi
----
...
[source,text]
----
build/jpackage/korTTY-2.0.0.deb
----

*Installation:*
[source,bash]
----
sudo dpkg -i build/jpackage/korTTY-2.0.0.deb
----
...
[source,text]
----
build/jpackage/korTTY-2.0.0.rpm
----

*Installation:*
[source,bash]
----
sudo rpm -i build/jpackage/korTTY-2.0.0.rpm
----
...
* If you use an older binary, rebuild or update to `v2.0.0+` to get the full target language list in *Settings → Translation*.
...
3. Optionally tune *Max characters*, *Tokenizer*, *Token limit*, warning thresholds, and reset cycle per profile.
...
The base command name is configurable. A separate setting can make the command name case-insensitive. KorTTY installs the shortcuts as shell aliases and emits a hidden terminal marker instead of sending the command to the remote shell as a normal executable.

For terminal-targeted agent runs, the inline activity panel at the bottom of the terminal shows the current task, token usage when reported by the endpoint, command/read actions, collapsible details, previous/next run navigation, rerun, close, collapse, resize, font controls, export controls, and a red cancel button. While the agent is running, normal terminal input is locked; *Esc*, *Ctrl+C*, or the panel cancel button aborts the run.
...
The shell shortcut captures the current remote directory with `pwd -P` and sends it to KorTTY. Agent commands then run relative to that directory, so generated files are created where the user is currently working on the server. If sudo is required, KorTTY asks for the password in the activity panel with a masked input field instead of echoing it in the terminal.
...
The documentation uses Mermaid diagrams (`.mmd` files in `docs/diagrams/`). To regenerate PNG images from them (e.g. after editing a diagram), use the provided script. Requires Node.js and npx.

* Render all diagrams: `./scripts/render-mermaid.sh` or `./scripts/render-mermaid.sh all`
* Render specific diagrams: `./scripts/render-mermaid.sh architecture connection-flow`

PNGs are written to `docs/diagrams/` and are embedded in the AsciiDoc documentation.

*CI and releases:* The build-release workflow runs the render script in the `docs-render` job before other steps. The generated PNGs are uploaded as the `docs-diagrams` artifact and, on release, packaged into `KorTTY-docs-diagrams-<version>.zip` on the GitHub Release. The workflow now also uses current Node-24-ready GitHub Actions majors for checkout, Node setup, and artifact upload. Use the diagram zip for doc publication or to get the diagrams without running the render script locally.
