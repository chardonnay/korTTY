...
*v2.4.1* | link:https://github.com/chardonnay/korTTY/releases/latest[Latest release] — A modern SSH client with JavaFX interface, tabbed terminals, local-shell sessions (Windows cmd/PowerShell or your `$SHELL`), SFTP tooling, background job scheduling, and OpenAI-compatible AI assistance.
...
* *Rendered Images, Diagrams & Math in AI Chats*: AI answers render inline as images — SVG documents (sanitized, JavaScript disabled), base64 raster images (PNG/JPEG/GIF/BMP data URIs), PlantUML diagrams (local `java` + Graphviz toolchain), Mermaid diagrams and LaTeX formulas (```latex/```math blocks and `$$…$$` in prose) via bundled Mermaid/MathJax without network access — each with a *Show code / Show image* toggle and copy button
...
On first run, Gradle will clone link:https://github.com/chardonnay/SithTermFX[SithTermFX] at tag `v1.2.0` into `vendor/sithtermfx`, build it with Maven, and install it to your local Maven repo. Subsequent builds reuse the clone unless you remove `vendor/sithtermfx`.
...
To *manually trigger* a release build (e.g. after moving the tag without creating a new release), go to *Actions → Build Release Binaries → Run workflow*, choose branch `main`, set *Ref* to the tag (e.g. `v2.2.2`) and *Version* to the numeric version (e.g. `2.2.2`).

== SithTermFX Integration

KorTTY uses *SithTermFX 1.2.0*, built from source. No GitHub token required (local or CI).

* *Local build:* The first time you run `./gradlew build`, the task `cloneSithtermfx` clones the link:https://github.com/chardonnay/SithTermFX[SithTermFX] repo at tag `v1.2.0` into `vendor/sithtermfx`, then `installSithtermfxLocal` runs `mvn -q -DskipTests install` there. The artifacts are installed to your local Maven repo (`mavenLocal()`). Requirements: *Git* and *Maven* on your PATH.
* *CI (GitHub Actions):* The workflow clones SithTermFX at tag `v1.2.0` into `vendor/sithtermfx`, installs Maven, then Gradle runs the same `installSithtermfxLocal` task. No GitHub token required.
...
| *Snippet Diagrams* | PlantUML renderer used by the Snippet Editor diagram feature | 1.2026.2
...
SithTermFX is built from source (see <<#sithtermfx-integration,SithTermFX Integration>>). pty4j backs both the native Mosh client and the <<#local-shell,Local Shell>> feature (spawning `cmd.exe`/PowerShell or `$SHELL` in a local PTY). mosh4j JARs are bundled in release builds so Mosh works without a first-run download. mosh4j `2.0.2` is a security-hardened release (replay/freshness protection, decompression limits). Its dynamically loaded classpath ships with Bouncy Castle `bcprov-jdk18on` `1.84` and `protobuf-java` `4.35.1` (the version mosh4j's generated DTOs are compiled against). The GitHub release tag is `v2.0.2` while the artifact files remain versioned `2.0.2`.
...
[source,text]
----
build/jpackage/korTTY-2.2.2.dmg
----
...
[source,text]
----
build\jpackage\korTTY-2.2.2.msi
----
...
[source,text]
----
build/jpackage/korTTY-2.2.2.deb
----

*Installation:*
[source,bash]
----
sudo dpkg -i build/jpackage/korTTY-2.2.2.deb
----
...
[source,text]
----
build/jpackage/korTTY-2.2.2.rpm
----

*Installation:*
[source,bash]
----
sudo rpm -i build/jpackage/korTTY-2.2.2.rpm
----
...
** *In-app logo*: `kortty_logo.png` is used in the login and About dialogs, with `kortty_icon.png` as runtime fallback.
...
* If you use an older binary, rebuild or update to `v2.2.2+` to get the full target language list in *Settings → Translation*.
...
* Optionally includes a PlantUML diagram
...
* *Summary* — a plain-language description of what the script does
* *Improvements* — suggestions grouped into *Security*, *Optimization* and *Design*, each with a severity badge, explanation and concrete recommendation; tick the ones to apply (or *Select all*)
...
* *Flow diagram* — an auto-generated diagram of the script logic with the full toolbar (zoom, fit, save SVG/PNG, copy image/PlantUML, *Dark mode*, background colour, regenerate) that highlights the matching source lines when you hover a node
...
* *Hardening options* — a collapsible panel (see below) whose techniques are baked into the applied fixes
* *Apply selected* — sends the ticked items to the AI in one request and shows a side-by-side before/after diff with per-change reasons before anything changes

The AI-code windows also share a per-run AI-profile picker with a *Re-run* button and remembered font sizes, and the editor pre-selects relevant AI Skills that you can pin so they apply to every AI-code action.
