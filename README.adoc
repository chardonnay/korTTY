...
* *Rendered Images, Diagrams & Math in AI Chats*: AI answers render inline as images — SVG documents (sanitized, JavaScript disabled), base64 raster images (PNG/JPEG/GIF/BMP data URIs), locally rendered PlantUML diagrams, Mermaid diagrams and LaTeX formulas (```latex/```math blocks and `$$…$$` in prose) via bundled Mermaid/MathJax without a remote renderer — each with a *Show code / Show image* toggle and copy button. Packaged PlantUML rendering reuses korTTY's private launcher worker; Graphviz is optional and needed only by diagram types that rely on `dot`.
...
| | Graphviz `dot` for PlantUML diagram types that require it | external
|===
