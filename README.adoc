...
*v2.18.0* | link:https://github.com/chardonnay/korTTY/releases/latest[Latest release] — A modern SSH client with JavaFX interface, tabbed terminals, local-shell sessions (Windows cmd/PowerShell or your `$SHELL`), SFTP tooling, background job scheduling, and OpenAI-compatible AI assistance.
...
* *App Designs*: Choose among all seven AtlantaFX 2.1.0 themes or korTTY's built-in designs in *Settings → Appearance* for JavaFX windows and dialogs without changing terminal-session or editor-content colors; new installations start with *AtlantaFX Primer Dark*
...
* *Coding Agents*: Detects Claude Code, Codex and Gemini CLI running in local shell tabs and tracks whether each pane is working, blocked on a question, done or idle — shown as tab-title glyphs, dashboard chips and accents, a dockable Coding Agents panel with one-click answers (y/n/Enter/Esc/Ctrl+C) and a prompt box, a status-bar strip, an app-icon badge with the number of waiting agents and desktop notifications for panes you are not looking at; analysed locally, switchable in *Settings → Terminal → Coding agents*, rules overridable under ~/.kortty/coding-agents/<agent>.json
...
* *Server Tags*: Optional free-text tag per connection — shown as a badge in the Connection Manager tree and Quick Connect, matched by every connection search, assigned or removed in bulk via the context menu (multi-selection and whole folders), with tag-based selection in the connection export dialog
...
* *Linux*: `-x86_64` or `-aarch64` (separate Flatpak/DEB/RPM/tar.gz/zip files). Install a downloaded Flatpak bundle with `flatpak install --user ./kortty-Linux-<version>-<architecture>.flatpak`.
* *Arch Linux*: `-x86_64` pacman `.pkg.tar.zst` packages for Intel/AMD Arch Linux.
* *Arch Linux ARM*: `-aarch64` pacman `.pkg.tar.zst` packages for AArch64 devices.
...
To *manually trigger* a release build, go to *Actions → Build Release Binaries → Run workflow*. Set *Source ref* to the exact branch, tag or commit to build, *Release tag* to the existing GitHub release that receives the assets (or `ci-only` for an artifact-only test), and *Version* to the matching numeric version without the leading `v`. Choose `flatpak-only` to build and upload only the two Flatpak assets without replacing existing release files. Choose `pacman-only` for a controlled Pacman backfill: provide the new package release number and the immutable commit expected behind the source release tag, inspect the two Actions artifacts with `release_tag=ci-only`, then repeat with the real release tag to publish without replacing existing assets.
...
| | BouncyCastle (bcprov, bcpkix) — also provides Ed25519/EdDSA | 1.85.2
...
| | Tukaani xz | 1.9
| *UI / Editor* | Monaco Editor | 0.56.0
| | Mermaid | 11.16.0
| | MathJax | 3.2.2
