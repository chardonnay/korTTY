...
* *AI Result Tabs*: Continue with follow-up prompts, export to PDF/Markdown/Text, copy detected code blocks, and persist the AI tab font size across sessions
...
* AI result tabs support follow-up prompts, export to PDF/Markdown/Text, code-block copy helpers, request cancellation, and a font size that is reused for future AI tabs.
...
*CI and releases:* The build-release workflow runs the render script in the `docs-render` job before other steps. The generated PNGs are uploaded as the `docs-diagrams` artifact and, on release, packaged into `KorTTY-docs-diagrams-<version>.zip` on the GitHub Release. Use that zip for doc publication or to get the diagrams without running the render script locally.
