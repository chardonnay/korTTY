...
* *AI Profiles & Quotas*: Manage multiple named AI profiles with per-profile endpoint/model/API key, maximum selection size, tokenizer strategy, token budgets, warning thresholds, and reset cycles
* *AI Result Tabs*: Continue with follow-up prompts, save named chats, share/export to PDF/Markdown/Text, copy detected code blocks and markdown tables, and persist the AI tab font size across sessions
* *AI Chat Manager*: Reopen, rename, refresh, and delete saved AI chats via *Tools → AI Manager* (shortcut: *Ctrl/Cmd+Shift+Y*)
