...
To *manually trigger* a release build, go to *Actions → Build Release Binaries → Run workflow*. Set *Source ref* to the exact branch, tag or commit to build, *Release tag* to the existing GitHub release that receives the assets, and *Version* to the matching numeric version without the leading `v`. Choose `flatpak-only` to build and upload only the two Flatpak assets without replacing existing release files.
