...
* *Coding Agent Detection*: Detects Claude Code, Codex and Gemini CLI running in local shell tabs and tracks whether each pane is working, blocked on a question, or idle — analysed locally, switchable in *Settings → Terminal → Coding agents*, rules overridable under ~/.kortty/coding-agents/<agent>.json
