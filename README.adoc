...
* *AI Terminal Actions*: Right-click selected terminal text to summarize output or analyze problems via an OpenAI-compatible API (including local tools such as LM Studio)
...
KorTTY can send selected terminal text to an OpenAI-compatible AI endpoint and open the result in a temporary tab.

1. Open *Settings → AI*.
2. Enter the *API URL*. *Model* and *API key* are optional so local OpenAI-compatible endpoints such as *LM Studio* can be used without both fields.
3. Click *Test AI Connection* to verify the endpoint.
4. In a terminal, mark the text you want to analyze.
5. Right-click the selected text and choose *AI → Summarize* or *AI → Solve Problem*.
6. Confirm the request in the preview dialog. The result opens in a new temporary AI tab.

Notes:

* AI result tabs are not saved with projects or session restore.
* `<think> ... </think>` blocks are stripped from the visible AI output.
* The font size in AI result tabs can be changed directly in the tab toolbar and is reused for future AI tabs.
