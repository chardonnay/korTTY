...
*v2.1.0* — A modern SSH client with JavaFX interface, tabbed terminals, SFTP tooling, and OpenAI-compatible AI assistance.
...
To *manually trigger* a release build (e.g. after moving the tag without creating a new release), go to *Actions → Build Release Binaries → Run workflow*, choose branch `main`, set *Ref* to the tag (e.g. `v2.1.0`) and *Version* to the numeric version (e.g. `2.1.0`).
...
[source,text]
----
build/jpackage/korTTY-2.1.0.dmg
----
...
[source,text]
----
build\jpackage\korTTY-2.1.0.msi
----
...
[source,text]
----
build/jpackage/korTTY-2.1.0.deb
----

*Installation:*
[source,bash]
----
sudo dpkg -i build/jpackage/korTTY-2.1.0.deb
----
...
[source,text]
----
build/jpackage/korTTY-2.1.0.rpm
----

*Installation:*
[source,bash]
----
sudo rpm -i build/jpackage/korTTY-2.1.0.rpm
----
...
* If you use an older binary, rebuild or update to `v2.1.0+` to get the full target language list in *Settings → Translation*.
