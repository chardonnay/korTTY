...
* *AI assistant for terminal selections* with profile-based OpenAI-compatible endpoints, follow-up prompts, and token quota tracking
...
2. Go to *AI*.
3. Create one or more AI profiles and enter the *API URL* for each profile you want to use.
...
7. Optionally disable the confirmation dialog for *Summarize* and *Solve Problem* if you want a faster workflow. *Ask* always opens the prompt dialog.
...
* The toolbar lets you copy the conversation, save or rename the chat, share/export it to PDF/Markdown/plain text, close the tab, cancel running requests, and change the font size.
* Detected code blocks get their own copy button.
...
* `connections.xml`, `credentials.xml`, `ssh-keys.xml`, `gpg-keys.xml`, `global-settings.xml`, `snippets.xml`, `snippet-variables.xml`, `master-password-hash`, `kortty.log`
