...
* *AI assistant for terminal selections* with profile-based OpenAI-compatible endpoints, follow-up prompts, AI Agent / Planning workflows, and token quota tracking
...
9. Optionally configure the agent command name, execution target, prompt-hook usage, and debug/runtime visibility for *AI Agent* and *AI Planning*.
...
* *AI Agent* - Start from *Tools > AI Agent...* or from the terminal right-click menu. The agent can open in a dedicated chat tab or target the active terminal window, depending on *Settings > AI*.
* *AI Planning* - Start from *Tools > AI Planning...* or from the terminal right-click menu. Planning mode asks clarifying questions, proposes one or more options, and lets you start the accepted plan.
* *Approvals* - The agent can request explicit approval before risky commands and can prompt for sudo passwords when needed.
* *Transcripts* - Agent and planning tabs can copy and save their transcript.

==== Terminal agent shortcut commands

When *Settings > AI > Use OSC 133 prompt markers* is enabled and the terminal is at a shell prompt, KorTTY intercepts these commands locally instead of sending them to the server:
...
The base command name is configurable in *Settings > AI*. If you rename `agent`, KorTTY derives the matching `-ask` and `-plan` commands automatically.
...
Key tabs: *Font and Colors*, *Terminal* (dimensions, scrollback, encoding, SSH Keep-Alive, close-confirmation behavior for active terminal windows), *Snippet Editor*, *Backup*, *Window* (geometry, dashboard state, menu bar visibility), *Security* (master password), *Language*, *Translation* (dynamic language files), *AI* (OpenAI-compatible profiles, default profile, agent/planning runtime options, snippet-AI behavior switches, saved-chat compatibility, and token quotas), and *Themes* (color profiles with live preview).
