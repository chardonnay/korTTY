...
*v2.0.0* | link:../README.adoc[Back to README]
...
| macOS (.dmg) | `./gradlew jpackageDmg` | `build/jpackage/korTTY-2.0.0.dmg`
| Windows (.exe) | `gradlew.bat jpackage` | `build\jpackage\korTTY\`
| Windows (.msi) | `gradlew.bat jpackageMsi` | `build\jpackage\korTTY-2.0.0.msi`
| Linux (AppImage) | `./gradlew jpackage` | `build/jpackage/korTTY/`
| Linux (.deb) | `./gradlew jpackageDeb` | `build/jpackage/korTTY-2.0.0.deb`
| Linux (.rpm) | `./gradlew jpackageRpm` | `build/jpackage/korTTY-2.0.0.rpm`
|===
...
5. Optionally configure *Max characters*, *Tokenizer*, *Token limit*, warning thresholds, and the token reset cycle per profile.
...
* *AI Planning* - Start from *Tools > AI Planning...*, from the terminal right-click menu, or with `agent-plan`. Planning mode asks clarifying questions, proposes one or more options, and lets you start the accepted plan.
...
* *Panel content* - The panel shows agent messages, read/run actions, task timing, reported token usage, semantic activity markers, collapsible details, previous/next run navigation, rerun, close, collapse/expand, resize, font-size controls, and export controls.
...
* *Current directory* - Terminal shortcut runs capture the current remote directory with `pwd -P` and pass it to the agent. Commands and generated files are executed relative to that directory.
* *Approvals and sudo* - The agent can request explicit approval before command execution and can ask for a sudo password in the activity panel. Password input is masked; if a password is cached, it is used only for the current agent/session context.
...
KorTTY installs these shortcuts as shell aliases that emit a hidden `OSC 777` marker back to the application. The marker includes the requested agent mode, the user text, and the current remote directory from `pwd -P`. The startup alias itself is removed from Bash history after installation, while user-entered agent commands remain available in shell history.
...
Key tabs: *Font and Colors*, *Terminal* (dimensions, scrollback, encoding, SSH Keep-Alive, close-confirmation behavior for active terminal windows), *Snippet Editor*, *Backup*, *Window* (geometry, dashboard state, menu bar visibility), *Security* (master password), *Language*, *Translation* (dynamic language files), *AI* (OpenAI-compatible profiles, default profile, agent command matching, terminal-agent dialog behavior, agent/planning runtime options, terminal-agent panel preferences, snippet-AI behavior switches, saved-chat compatibility, and token quotas), and *Themes* (color profiles with live preview, including dedicated Terminal AI agent panel colors).
