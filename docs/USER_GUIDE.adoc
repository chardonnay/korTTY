...
*v1.9.0* | link:../README.adoc[Back to README]
...
4. Confirm the request in the preview dialog. The response opens in a temporary AI tab.

*AI result tab behavior:*

* The tab is read-only and not included in saved project/session state.
* `<think> ... </think>` blocks are removed from the visible output.
* The toolbar lets you copy the response, close the tab, and change the font size.
...
Key tabs: *Font and Colors*, *Terminal* (dimensions, scrollback, encoding, SSH Keep-Alive), *Snippet Editor*, *Backup*, *Window* (geometry, dashboard state), *Security* (master password), *Language*, *Translation* (dynamic language files), *AI* (OpenAI-compatible terminal analysis), and *Themes* (color profiles with live preview).
