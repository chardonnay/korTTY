...
* *Code snippet manager* with syntax highlighting, line numbers, one-liner export, placeholder variables, and import/export
...
* *AI assistant for terminal selections* with profile-based OpenAI-compatible endpoints, follow-up prompts, Terminal AI Agent / Planning workflows, inline activity feed, and token quota tracking
...
9. Optionally configure the agent command name, case-insensitive command matching, execution target, prompt-hook usage, per-run setup dialog, and debug/runtime visibility for *AI Agent* and *AI Planning*.
...
* *Inline activity panel* - Terminal-targeted runs appear at the bottom of the active terminal tab. The panel shows agent messages, read/run actions, task timing, reported token usage, collapsible details, previous/next run navigation, rerun, close, collapse/expand, resize, font-size controls, and export controls.
* *Input lock and cancel* - While the agent is active, normal terminal input is blocked for that terminal tab. Press *Esc*, press *Ctrl+C*, or use the red cancel button in the panel to abort the run.
...
* Use *Expand all* to keep activity details open instead of compacting them.
...
Key tabs: *Font and Colors*, *Terminal* (dimensions, scrollback, encoding, SSH Keep-Alive, close-confirmation behavior for active terminal windows), *Snippet Editor*, *Backup*, *Window* (geometry, dashboard state, menu bar visibility), *Security* (master password), *Language*, *Translation* (dynamic language files), *AI* (OpenAI-compatible profiles, default profile, agent command matching, terminal-agent dialog behavior, agent/planning runtime options, snippet-AI behavior switches, saved-chat compatibility, and token quotas), and *Themes* (color profiles with live preview, including dedicated Terminal AI agent panel colors).
