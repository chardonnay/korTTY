...
5. Optionally configure *Max characters*, *Tokenizer*, *Token limit*, warning thresholds, token reset cycle, and supported *Reasoning* effort per profile. KorTTY exposes reasoning choices based on the configured API URL and model; profiles without a supported reasoning mode stay disabled.
